...
public class HandController : MonoBehaviour {
  ...
  */
  ...
  public List<int> objects_id;
  //public GameObject prefab_file;
  ...
  void Update() {
    ...
    for (int i = 0; i < gestures.Count; i++) {
      Gesture gesture = gestures [i];
			Debug.Log (i + gesture.Type + " " + Gesture.GestureType.TYPE_CIRCLE);
			if (gesture.Type == Gesture.GestureType.TYPE_CIRCLE) {
				...
				// counterclockwise means create a new object
				if ( circle.Pointable.Direction.AngleTo (circle.Normal) > Math.PI / 2 ) {
					float sweptAngle = 0;
					if (circle.State != Gesture.GestureState.STATE_START) {
						CircleGesture previousUpdate = new CircleGesture (leap_controller_.Frame (1).Gesture (circle.Id));
                        sweptAngle = (circle.Progress - previousUpdate.Progress) * 360;
						//SafeWriteLine("Create a new object" + circle.Id);
						Debug.Log ("In here" + circle.Id);
						GameObject go = GameObject.Find("Rotating_Cube");
						if (go != null) {
							//SafeWriteLine("Find origin object");
							GameObject clone = Instantiate(go, 
                        										 go.transform.position, 
                        										 go.transform.rotation)
																 as GameObject;
							clone.transform.localScale = new Vector3(1, 1, 1);
							clone.transform.Translate(0, 1, 3);
						}

					  objects_id.Add(circle.Id);
					}
				}
			}
    }
